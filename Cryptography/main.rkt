...
(define (crack-cipher strategies key)
  (if(null? strategies) '()                                                                            ;;if strategies are over return '()
     (crack-cipher-helper ((car strategies) key) strategies key) 
  ...
  (define (crack-cipher-helper list-of-subst list-of-strategies key)                            
     ;(utils:show-key (utils:add-substitution list-of-subst (make-list 26 #\_)) )          
    (if(null? list-of-subst) (crack-cipher (cdr list-of-strategies) key)                              
       (cond[(not (algo:dictionary-closure (utils:add-substitution (car list-of-subst) key))) (crack-cipher-helper (cdr list-of-subst) list-of-strategies key)]
            [else (cond[(not (algo:secret-word-enumeration (algo:dictionary-closure (utils:add-substitution (car list-of-subst) key)))) (crack-cipher-helper (cdr list-of-subst) list-of-strategies key)]
                       [else (cons (algo:secret-word-enumeration (algo:dictionary-closure (utils:add-substitution (car list-of-subst) key))) (crack-cipher-helper (cdr list-of-subst) list-of-strategies key))])]))) 
